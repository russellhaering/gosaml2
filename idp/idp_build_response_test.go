...
func TestBuildResponseDocument(t *testing.T) {
	...
	params := &AssertionParams{
		...
		AuthnContext: saml2.AuthnContextPasswordProtectedTransport,
...
func TestBuildErrorResponseDocument(t *testing.T) {
	...
	doc, err := idp.BuildErrorResponseDocument("https://sp.test/metadata", saml2.StatusCodeRequester, "_req123", "https://sp.test/acs")
	require.NoError(t, err)
	require.NotNil(t, doc)

	xmlStr, err := doc.WriteToString()
	require.NoError(t, err)
	require.Contains(t, xmlStr, saml2.StatusCodeRequester)
...
func TestBuildErrorResponseDocument_UnknownSP(t *testing.T) {
	...
	_, err := idp.BuildErrorResponseDocument("https://unknown.test", saml2.StatusCodeRequester, "_req123", "https://sp.test/acs")
	require.Error(t, err)
	require.ErrorIs(t, err, saml2.ErrUnknownSP)
}
...
func TestResponseRoundtrip(t *testing.T) {
	...
	doc, acsURL, err := idpInst.BuildResponseDocument("https://sp.test/metadata", params)
	require.NoError(t, err)
	require.Equal(t, "https://sp.test/acs", acsURL)
	...
	response, err := spInst.ValidateEncodedResponse(context.Background(), encodedResponse)
	require.NoError(t, err)
	...
	require.Equal(t, saml2.NameIdFormatEmailAddress, assertion.Subject.NameID.Format)
...
func TestResponseRoundtrip_WithInResponseTo(t *testing.T) {
	...
	tracker := sp.NewMemoryRequestTracker(10 * time.Minute)
