...
func TestEncryptAssertion_NoCert(t *testing.T) {
	idpInst, _ := testIdentityProvider(t)
	...
	assertionEl := idpInst.buildAssertion(spConfig, params, "https://sp.test/acs")

	_, err := idpInst.encryptAssertion(assertionEl, spConfig)
	require.Error(t, err)
	require.ErrorIs(t, err, saml2.ErrEncryptionFailed)
}
