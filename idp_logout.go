...
// received via the HTTP-Redirect binding.
...
// BuildLogoutResponseDocument builds a SAML LogoutResponse document.
...
// BuildLogoutResponseBodyPost builds the POST form body for a SAML LogoutResponse.
func (idp *IdentityProvider) BuildLogoutResponseBodyPost(spEntityID, statusCode, inResponseTo, destination, relayState string) ([]byte, error) {
	...
	return buildPOSTForm(destination, "SAMLResponse", base64.StdEncoding.EncodeToString(docBytes), relayState)
}

// BuildLogoutRequestDocument builds a SAML LogoutRequest document to send to an SP.
func (idp *IdentityProvider) BuildLogoutRequestDocument(spEntityID, nameID, nameIDFormat, sessionIndex string) (*etree.Document, error) {
	...
	logoutRequestEl.CreateAttr("xmlns:samlp", SAMLProtocolNamespace)
	logoutRequestEl.CreateAttr("xmlns:saml", SAMLAssertionNamespace)
	logoutRequestEl.CreateAttr("ID", requestID)
	logoutRequestEl.CreateAttr("Version", "2.0")
	logoutRequestEl.CreateAttr("IssueInstant", now.Format(issueInstantFormat))
	if destination != "" {
