...
- **Secure by default.** SHA-1 is rejected, signatures are required (including on LogoutRequests, over both bindings), IDP-initiated SSO is off, audience scoping and replay protection are required rather than optional where they are the only bound on misuse, a signed logout message must name the endpoint it was delivered to, and condition violations are hard errors. A security control whose configuration is absent fails closed rather than silently not running. Insecure behavior requires explicit opt-in via a named `Insecure*` or `Allow*` field.
...
- **Published metadata states what is enforced.** An identity provider advertises `WantAuthnRequestsSigned` only when every service provider it knows about actually requires signed AuthnRequests, and `ConfigureFromSPMetadata` carries a service provider's declared `AuthnRequestsSigned` into that requirement — so onboarding an SP from its metadata does not silently drop the signing guarantee the SP publishes, and the metadata never promises a check that will not run.
