...
- **Secure by default.** SHA-1 is rejected, signatures are required (including on LogoutRequests, over both bindings), IDP-initiated SSO is off, replay protection is required rather than optional where it is the only bound on reuse, and condition violations are hard errors. Insecure behavior requires explicit opt-in via a named `Insecure*` or `Allow*` field.
...
- **Logout message single-use**: logout messages are bounded by age (`MaxIssueInstantAge`, default 5 minutes) and by `NotOnOrAfter` when present, which bounds but does not eliminate replay inside that window. An application needing strict single-use should track logout message IDs itself.
