...
func (e RSA) Encrypt(certificate interface{}, plaintext []byte, nonce []byte) ([]*etree.Element, error) {
	...
	encryptedKeyEl := etree.NewElement("xenc:EncryptedKey")
	encryptedKeyRefID, err := getReferenceID()
	if err != nil {
		return nil, err
	}
	encryptedKeyEl.CreateAttr("Id", encryptedKeyRefID)
	encryptedKeyEl.CreateAttr("xmlns:xenc", "http://www.w3.org/2001/04/xmlenc#")

	encryptionMethodEl := encryptedKeyEl.CreateElement("xenc:EncryptionMethod")
	...
	cd := encryptedKeyEl.CreateElement("xenc:CipherData")
	...
	encryptedDataRefID := encryptedDataEl.SelectAttrValue("Id", "")

	refList := encryptedKeyEl.CreateElement("xenc:ReferenceList")
	dataRef := refList.CreateElement("xenc:DataReference")
	dataRef.CreateAttr("URI", "#"+encryptedDataRefID)

	keyInfoEl := etree.NewElement("ds:KeyInfo")
	keyInfoEl.CreateAttr("xmlns:ds", "http://www.w3.org/2000/09/xmldsig#")

	retrievalMethod := keyInfoEl.CreateElement("ds:RetrievalMethod")
	retrievalMethod.CreateAttr("Type", "http://www.w3.org/2001/04/xmlenc#EncryptedKey")
	retrievalMethod.CreateAttr("URI", "#"+encryptedKeyRefID)

	encryptedDataEl.InsertChildAt(encryptedDataEl.FindElement("./CipherData").Index(), keyInfoEl)

	return []*etree.Element{encryptedDataEl, encryptedKeyEl}, nil
}
...
// the block cipher used is AES-128 CBC. The DigestMethod field is ignored because PKCS1v15
...
func getReferenceID() (string, error) {
	randBuf := make([]byte, 16)
	if _, err := RandReader.Read(randBuf); err != nil {
		return "", err
	}
	return fmt.Sprintf("_%x", randBuf), nil
}
