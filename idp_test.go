...
func testIdPKeyStore(t *testing.T) *KeyStore {
	t.Helper()
	return testKeyStore(t, testTime)
}

func testSPKeyStore(t *testing.T) (*KeyStore, *x509.Certificate) {
	t.Helper()
...
func testIdentityProvider(t *testing.T) (*IdentityProvider, *KeyStore) {
	t.Helper()
...
func TestIdentityProvider_LookupSP(t *testing.T) {
	...
	sp, err := idp.lookupSP("https://sp.test/metadata")
	require.NoError(t, err)
	...
	ed := &types.EntityDescriptor{
		...
		SPSSODescriptor: &types.SPSSODescriptor{
			AssertionConsumerServices: []types.IndexedEndpoint{
				{Binding: BindingHttpPost, Location: "https://sp.test/acs", Index: 1},
			},
			SingleLogoutServices: []types.Endpoint{
				{Binding: BindingHttpPost, Location: "https://sp.test/slo"},
			},
		},
	}
...
func TestIdentityProvider_Metadata(t *testing.T) {
	...
	require.Equal(t, SAMLProtocolNamespace, md.IDPSSODescriptor.ProtocolSupportEnumeration)
	...
	require.Equal(t, NameIdFormatTransient, md.IDPSSODescriptor.NameIDFormats[0].Value)
...
func TestIdentityProvider_Metadata_CustomNameIDFormats(t *testing.T) {
	idp, _ := testIdentityProvider(t)
	idp.NameIDFormats = []string{NameIdFormatEmailAddress, NameIdFormatPersistent}

	md, err := idp.Metadata()
	require.NoError(t, err)
	require.Len(t, md.IDPSSODescriptor.NameIDFormats, 2)
	require.Equal(t, NameIdFormatEmailAddress, md.IDPSSODescriptor.NameIDFormats[0].Value)
	require.Equal(t, NameIdFormatPersistent, md.IDPSSODescriptor.NameIDFormats[1].Value)
}
