...
type idpFormatTestCase struct {
	...
	xmlFile     string
	idpEntityID string
	checkResult func(t *testing.T, info *AssertionInfo)
}

// newIdPFormatSP creates a ServiceProvider configured for IdP format tests.
// The SP trusts the given certificate, uses a fixed fake clock, and expects
// responses sent to http://localhost:8080/v1/_saml_callback with the audience
// https://sp.example.com.
func newIdPFormatSP(t *testing.T, ks *KeyStore, idpCert *x509.Certificate, idpEntityID string) *ServiceProvider {
	t.Helper()
...
func loadAndSign(t *testing.T, path string, sp *ServiceProvider) string {
	t.Helper()
	xmlBytes, err := os.ReadFile(path)
	require.NoError(t, err)
	signed := signResponse(t, string(xmlBytes), sp)
...
func TestIdPResponseFormats(t *testing.T) {
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			sp := newIdPFormatSP(t, ks, idpCert, tc.idpEntityID)
			encoded := loadAndSign(t, tc.xmlFile, sp)

			ctx := context.Background()

			// Validate the response can be parsed
			resp, err := sp.ValidateEncodedResponse(ctx, encoded)
			require.NoError(t, err)
...
func TestDecodeUnverifiedBaseResponse_IdPFormats(t *testing.T) {
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			...
			resp, err := DecodeUnverifiedBaseResponse(encoded)
			require.NoError(t, err)
			require.NotNil(t, resp)
